...
import { MessageSquare, Map, HelpCircle, LogOut, User, Building } from 'lucide-react';
...
export default function StudentDashboard() {
    ...
    const navItems = [
        { path: '/student/dashboard', label: 'Course Chat', icon: <MessageSquare size={20} /> },
        { path: '/student/roadmaps', label: 'Roadmaps', icon: <Map size={20} /> },
        { path: '/student/interview-prep', label: 'Interview Prep', icon: <HelpCircle size={20} /> },
        { path: '/student/insights', label: 'Company Insights', icon: <Building size={20} /> },
    ];
        ...
        <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-color)' }}>
            ...
            <aside style={{ width: '280px', background: 'rgba(255, 255, 255, 0.02)', borderRight: '1px solid var(--surface-border)', display: 'flex', flexDirection: 'column' }}>
                <div style={{ padding: '24px', borderBottom: '1px solid var(--surface-border)' }}>
                    <h2 className="text-gradient" style={{ fontSize: '1.5rem', marginBottom: '8px' }}>Student Hub</h2>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '16px' }}>
                        <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'var(--secondary-color)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <User size={20} color="white" />
                        </div>
                        <div>
                            <p style={{ fontWeight: '600', fontSize: '0.9rem' }}>{user?.full_name}</p>
                            <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>{user?.interested_course || 'No course selected'}</p>
                        </div>
                    </div>
                </div>
